...
const Dropdown = (props) => {
      ...
      if (menu.position) {
        if (menu.position.left) {
          menuElement.style.setProperty(
            "--position-left",
            (menu.position.left / width) * 100 + "%"
          );
        } else if (menu.position.right) {
          menuElement.style.setProperty(
            "--position-right",
            100 - (menu.position.right / width) * 100 + "%"
          );
        }
        if (menu.position.top) {
          menuElement.style.setProperty(
            "--position-top",
            (menu.position.top / height) * 100 + "%"
          );
        } else if (menu.position.bottom) {
          menuElement.style.setProperty(
            "--position-bottom",
            (menu.position.bottom / height) * 100 + "%"
          );
        }
      }
    }
  ...
  return (
    <div
      id={props.id}
      className={`${classes.dropdown} ${props.className || ""}`}
    >
      {props.children}
    </div>
  );
